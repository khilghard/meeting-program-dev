...
describe("HTML Syntax Validation", () => {
    ...
    test("index.html should use .catch() for async function calls", () => {
        ...
        // Check that registerServiceWorker is called with error handling
        const callPattern = /registerServiceWorker\(\)\s*(\.catch|await)/;
    ...
    test("All async functions should have error handling", () => {
        ...
        // Check that registerServiceWorker is called with error handling
        // Pattern: registerServiceWorker() should be followed by .catch() or wrapped in try-catch
        const callPattern = /registerServiceWorker\(\)(\s*\.catch\s*\(|\s*await)/;
        expect(content).toMatch(callPattern);
