...
test.describe("Test 02: Navigation & Comprehensive Data Verification", () => {
  test("should render all data keys and navigate to archives", async ({ page }) => {
    ...
    await mainPage.goto();
    await page.goto(
      `http://localhost:8000/meeting-program/?url=${encodeURIComponent(comprehensiveProgramUrl)}`
    );
    ...
    console.log(
      `✅ Keys found: ${requiredKeys.length - missingKeys.length}/${requiredKeys.length}`
    );
